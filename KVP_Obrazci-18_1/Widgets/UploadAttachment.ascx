<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UploadAttachment.ascx.cs" Inherits="KVP_Obrazci.UploadAttachment" %>
...
<script type="text/javascript">
    ...
    function onUploadControlFileUploadComplete(s, e) {
        ...
        $('#ctl00_ContentPlaceHolderMain_CallbackPanelKVPDocumentForm_test_documentContainer').append("<a class='uploaded-link' href='#' data-toggle='popover'" +
    ...
    function OnDropZoneEnter(s, e) {
        ...
        if (e.dropZone.id == dropZone)
            setElementVisible('#ctl00_ContentPlaceHolderMain_CallbackPanelKVPDocumentForm_test_externalDropZone', true);
    }
    function OnDropZoneLeave(s, e) {
        var dropZone = clientHfDropZone.Get('DropZone');
        if (e.dropZone.id == dropZone)
            setElementVisible('#ctl00_ContentPlaceHolderMain_CallbackPanelKVPDocumentForm_test_externalDropZone', false);
    }
...
<div style="display: block; width: 100%; overflow: hidden">
    ...
    <dx:ASPxUploadControl ID="UploadControl" runat="server" UploadMode="Auto" Width="100%"
        ...
        <AdvancedModeSettings EnableDragAndDrop="true" EnableFileList="false" EnableMultiSelect="true" ExternalDropZoneID="externalDropZone"
            DropZoneText="" />
        <ValidationSettings MaxFileSize="4194304" AllowedFileExtensions=".pdf,.jpg,.jpeg,.png,.csv,.xls,.xlsx" ErrorStyle-CssClass="validationMessage" />
