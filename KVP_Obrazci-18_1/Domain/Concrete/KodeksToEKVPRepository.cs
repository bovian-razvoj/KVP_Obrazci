...
namespace KVP_Obrazci.Domain.Concrete
{
    public class KodeksToEKVPRepository : IKodeksToEKVPRepository
    {
        ...
        string connString;
        public KodeksToEKVPRepository()
        {
            connString = ConfigurationManager.ConnectionStrings["KodeksOdelo"].ToString();
            departmentRepo = new DepartmentRepository(XpoHelper.GetNewSession());
            employeeRepo = new EmployeeRepository(XpoHelper.GetNewSession());
        }
        ...
        public List<KodeksUsersModel> GetKodeksUsers()
        {
            ...
            while (reader.Read())
            {
                list.Add(new KodeksUsersModel
                {
                    ...
                    Card = reader.IsDBNull(10) ? "NULL" : reader.GetString(10),
        ...
        private void MergeUsers(Session currentSession)
        {
            ...
            foreach (KodeksUsersModel kum in kodeksUsers)
            {
                ...
                AddNewEmployeetoeKVPFirstnameAndLastnameExternalId(kum.ExternalId, kum.Firstname, kum.Lastname, kum, currentSession);
            }
        }
        ...
        private void SetNameChangedByEternalID(string sExternalId, KodeksUsersModel kum, Session currentSession)
        {
            ...
            if (sExternalId.Length > 0)
            {
                ...
                if (usrColl.Count > 0)
                {
                    foreach (Users usr in usrColl)
                    {
                        if ((usr.Firstname.ToUpper() != kum.Firstname.ToUpper()) || (usr.Lastname.ToUpper() != kum.Lastname.ToUpper()))
                        {
                            if (usr.Deleted ==  true)
                            {
        ...
        private void SetLeftCompanyUserByFirstnameAndLastnameExternalId(string sExternalId, string sFirstName, string sLastname, KodeksUsersModel kum, Session currentSession)
        {
            ...
            if (usrColl.Count > 0)
            {
                foreach (Users usr in usrColl)
                {
                    if (kum.IsTimeAttendance != usr.IsTimeAttendance)
                    {
                        usr.IsTimeAttendance = kum.IsTimeAttendance;
                        usr.NotEmployedAnymore = 1;
                        usr.Save();
                    }
        ...
        /// <param name="kum"></param>
        /// <param name="currentSession"></param>
        private void AddNewEmployeetoeKVPFirstnameAndLastnameExternalId(string sExternalId, string sFirstName, string sLastname, KodeksUsersModel kum, Session currentSession)
        {
            CriteriaOperator filterCriteria = null;
            ...
            if (usrColl.Count == 0)
            {
                ...
                newUsr.RoleID = employeeRepo.GetRoleByID(4, currentSession); ;
                newUsr.Save();
        ...
        {
            CriteriaOperator filterCriteria = CriteriaOperator.Parse("[Name] ='" + sDepartmentName.Trim().ToUpper() + "' and Deleted = 0");
