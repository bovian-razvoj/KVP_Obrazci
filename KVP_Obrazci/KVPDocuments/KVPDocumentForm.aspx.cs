    ...
    public partial class KVPDocumentForm : ServerMasterPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack || forwardBackKVPs)
            {
                ...
                ASPxHtmlEditorCIP.CreateDefaultToolbars(true);
                ASPxHtmlEditorCIP.Toolbars["StandardToolbar1"].Items.FindByCommandName("fullscreen").Visible = false;
        ...
        private void FillForm()
        {
            ...
            ASPxHtmlEditorCIP.Html = model.CIPOpombe;
            ...
            //if the kvp is in status Vnos than we can't change it!
            ...
            // 19.07.2019, email Mitja Berišaj: special request, if champion save KVP for emloyee can open and send this KVP to leader
            //if (model.LastStatusId != null && model.LastStatusId.Koda == Enums.KVPStatuses.VNOS.ToString() && PrincipalHelper.IsUserChampion() && model.IDPrijave == PrincipalHelper.GetUserPrincipal().ID)
            //{
            ...
            {
                //Če je kvp status v Čaka na odobritev vodja ali Poslano v presojo ali V realizaciji ali Realizirano, potem omogočimo zavihek za spreminjanje statusa
                if (AllowChampionToChangeStatus())
                {
        ...
        private void CustommizeByUserRole()
        {
            ...
            else if (PrincipalHelper.IsUserChampion())
            {
                ...
                //že kot privzeto nastavimo izbirne statuse za championa tudi če ne more spreminjati statusov
                XpoDSStatus.Criteria += " and [idStatus] in (14,2,8,4,9)";
            }
            ...
            {
                changeStatusItem.Style.Add("display", "block !important");
                cipItem.Style.Add("display", "block !important");
            }
        ...
        private bool AddOrEditEntityObject(bool add = false, Enums.KVPProcessStatus procStat = Enums.KVPProcessStatus.Nothing)
        {
            ...
            if (transferToRedCard)
            {
                ...
                model.RokOdziva = DateTime.Now.AddDays(14);
                model.PrenosRKizKVP = true;

                model.OpisNapakeRK = ASPxMemoProblemDesc.Text + "\r\n\r\n" + ASPxMemoImprovementProposition.Text;
            ...
            model.CIPOpombe = ASPxHtmlEditorCIP.Html;
            ...
            kvpDocID = model.idKVPDocument = kvpDocRepo.SaveKVP(model, submitProposalType, transferToRedCard);
            GetKVPDocumentProvider().SetKVPDocumentModel(model);
        ...
        protected void CallbackPanelKVPDocumentForm_Callback(object sender, CallbackEventArgsBase e)
        {
            if (e.Parameter == "SaveStatus")
            {
                ...
                bool isKVPRealizedAlready = kvpStatusRepo.HasKVPDocumentKVPStatus(model.idKVPDocument, Enums.KVPStatuses.REALIZIRANO.ToString());
                bool isKVPCompletedAlready = kvpStatusRepo.HasKVPDocumentKVPStatus(model.idKVPDocument, Enums.KVPStatuses.ZAKLJUCENO.ToString());

                if (status.idStatus.Koda == Enums.KVPStatuses.REALIZIRANO.ToString() && !isKVPRealizedAlready)
                    model.DatumRealizacije = DateTime.Now;
                else if (status.idStatus.Koda == Enums.KVPStatuses.ZAKLJUCENO.ToString() && !isKVPCompletedAlready)//če je v zgodovini statusa KVP-ja že status zaključeno potem onemogočimo prištevanje točk.
                {
                    model.DatumZakljuceneIdeje = DateTime.Now;
                    CompleteKVPDocument();
                }

                //če sta statusa različna dodamo nov zapis v tabelo. Drugače posodobimo obsotoječega.
                if (status.idStatus.idStatus == model.LastStatusId.idStatus)
                {
                    status = kvpStatusRepo.GetLatestKVPStatus(model.idKVPDocument);
                    status.Opomba = ASPxMemoOpombe.Text;
                }
            ...
            else if (e.Parameter == "PrintDocument")
            {
                CallbackPanelKVPDocumentForm.JSProperties["cpPrintID"] = ConcatenateURLForPrint(kvpDocID, "KVPDocument", false);
            }
        ...
        private void KVPProccessButtonsVisibility(int auditorsCount = 0)
        {
            ...
            if (model.LastStatusId.Koda == Enums.KVPStatuses.VNOS.ToString() || model.LastStatusId.Koda == Enums.KVPStatuses.ODOBRITEV_VODJA.ToString() ||
                model.LastStatusId.Koda == Enums.KVPStatuses.V_PREVERJANJE.ToString() ||
                model.LastStatusId.Koda == Enums.KVPStatuses.POSLANO_V_PRESOJO.ToString())
            ...
            if (model.LastStatusId.Koda == Enums.KVPStatuses.ZAVRNJEN.ToString())
            {
                ...
                rejectKVP = false;
                string arguments = "<h3>Argumenti zavrnitve</h3><p>Zavrnil: " +
                    (model.ZavrnitevIdUser != null ? model.ZavrnitevIdUser.Firstname + " " + model.ZavrnitevIdUser.Lastname : "") + "</p>" +
                    "<p>" + model.ZavrnitevOpis.Replace("\n", "<br />") + "</p>";
                Page.ClientScript.RegisterStartupScript(this.GetType(), "ShowRejectArguments", String.Format("ShowRejectModal(\"{0}\");", arguments), true);
            }
            ...
            if (model.LastStatusId.Koda == Enums.KVPStatuses.V_REALIZACIJI.ToString() && PrincipalHelper.IsUserChampion())
            {
                rejectKVP = true;
            }
        ...
        private void ClearSessionsAndRedirect(bool isIDDeleted = false, bool newKVPDoc = false, int? nextKVPdocId = null)
        {
            ...
            if (transferToRedCard)
            {
                queryStrings.Add(new QueryStrings() { Attribute = Enums.QueryStringName.transferToRedCard.ToString(), Value = "true" });
            }
        ...
        private void CompleteKVPDocument()
        {
            ...
            {
                //model.DatumZakljuceneIdeje = new DateTime(2019, 09, 03);
            }
            ...
            if (model.Predlagatelj.UpravicenDoKVP)
            {
                if (userProposerPayoutRecord != null)
                {
                    ...
                    // preverimo, če je slučajno PrenosIzPrejsnjegaMeseca = 0
                    if (userProposerPayoutRecord.PrenosIzPrejsnjegaMeseca == 0)
                    {
                        SetPrenosIzPrejsnjegaMeseca(userProposerPayoutRecord, true);
                    }
                ...
                {
                    payoutRepo.CreateNewPayout(model);
                }
            }
            ...
            if (model.Realizator.UpravicenDoKVP)
            {
                if (userRealizatorPayoutRecord != null)
                {
                    ...
                    // preverimo, če je slučajno PrenosIzPrejsnjegaMeseca = 0
                    if (userRealizatorPayoutRecord.PrenosIzPrejsnjegaMeseca == 0)
                    {
                        SetPrenosIzPrejsnjegaMeseca(userRealizatorPayoutRecord, false);
                    }

                    userRealizatorPayoutRecord.Save();
                }
                else
                {
                    payoutRepo.CreateNewPayout(model, true);
                }
            }
        ...
        private void SetPrenosIzPrejsnjegaMeseca(Izplacila userPayoutRecord, bool bIsPredlagatelj)
        {
            // preverimo če obstaja izračun za prejšnji mesec in prenesemo točke
            ...
            Izplacila payoutPreviousMonth = payoutRepo.UserPayoutRecordForMonthAndYear((bIsPredlagatelj ? model.Predlagatelj.Id : model.Realizator.Id), previousMonth, previousMonthDate.Year);
            if ((payoutPreviousMonth != null) && (payoutPreviousMonth.DatumIzracuna > new DateTime(2000, 1, 1)))
            {
                userPayoutRecord.PrenosIzPrejsnjegaMeseca = payoutPreviousMonth.PrenosTvNaslednjiMesec;
                userPayoutRecord.VsotaT = userPayoutRecord.VsotaT + userPayoutRecord.PrenosIzPrejsnjegaMeseca;
                payoutRepo.SavePayout(userPayoutRecord);
            }
        }
        ...
        public bool AllowChampionToChangeStatus()
        {
            string cakaOdobritev = Enums.KVPStatuses.ODOBRITEV_VODJA.ToString();
            string presoja = Enums.KVPStatuses.POSLANO_V_PRESOJO.ToString();
            string vRealizaciji = Enums.KVPStatuses.V_REALIZACIJI.ToString();
            string realizirano = Enums.KVPStatuses.REALIZIRANO.ToString();

            if (model != null && model.LastStatusId != null)
            {
                if (model.LastStatusId.Koda == cakaOdobritev || model.LastStatusId.Koda == presoja || model.LastStatusId.Koda == vRealizaciji || model.LastStatusId.Koda == realizirano)
                    return true;
            }
        ...
        protected void btnPrenosRdeciKarton_Click(object sender, EventArgs e)
        {
            ...
            ASPxMemoRejectKVPArguments.Text = "Zavrnitev KVP predloga zaradi prenosa v rdeči karton!";
            AddOrEditEntityObject(false, Enums.KVPProcessStatus.Reject);
