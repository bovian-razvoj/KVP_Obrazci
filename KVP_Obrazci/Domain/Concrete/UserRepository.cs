...
namespace KVP_Obrazci.Domain.Concrete
{
    public class UserRepository : IUserRepository
    {
        ...
        public UserModel UserLogIn(string userName, string password)
        {
            try
            {
                ...
                if (user != null)
                {
                    ...
                    if ((user.DepartmentId != null) && (user.DepartmentId.DepartmentHeadId == 0))
                    {
                        throw new EmployeeCredentialsException(AuthenticationValidation_Exception.res_05);
                    }
        ...
        public UserModel UserLogInCard(string token)
        {
            ...
            {
                XPQuery<Users> list = session.Query<Users>();
                Users user = list.Where(u => u.Card.CompareTo(token) == 0 && !u.Deleted).FirstOrDefault();
                UserModel model = null;
        ...
        private UserModel FillUserModel(Users user)
        {
            ...
            if (user.SecondRoleID != null)
            {
                model.SecondRoleID = user.SecondRoleID.VlogaID;
                model.SecondRole = user.SecondRoleID.Koda;
                model.SecondRoleName = user.SecondRoleID.Naziv;
            }
