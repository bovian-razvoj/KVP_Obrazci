...
namespace KVP_Obrazci.Domain.Concrete
{
    public class PlaningAndRealizationRepository : IPlaningAndRealizationRepository
    {
        ...
        private Int32 ReturnPodaniByKVPGroup(DateTime dtFrom, DateTime dtTo, Int32 idKVPGroup)
        {

            dtTo = new DateTime(dtTo.Year, dtTo.Month, dtTo.Day, 23, 59, 59);

            List<KVPGroupReportModel> list = kvpGroupRepo.GetKVPDocForDatePeriodLastStatusAndGroupID(dtFrom, dtTo, idKVPGroup, null, false);
        ...
        private void UpdateRealKVPCount(int year, Session currentSession = null)
        {
            try
            {
                CriteriaOperator filterCriteria = CriteriaOperator.Parse("Leto =" + year + " and [idKVPSkupina.Aktivnost] = 1 and ts <> '" + DateTime.Now.ToString("yyyy-MM-dd") + "'");

                XPCollection<PlanRealizacija> collection_KVP = new XPCollection<PlanRealizacija>(session, filterCriteria);
                collection_KVP.Sorting.Add(new SortProperty("idKVPSkupina", DevExpress.Xpo.DB.SortingDirection.Ascending));

                foreach (PlanRealizacija pln in collection_KVP)
                {
                    // TODO: Od Januarja do Junija
                    pln.Real_Jan = ReturnPodaniByKVPGroup(new DateTime(year, 1, 1), new DateTime(year, 1, 31), pln.idKVPSkupina.idKVPSkupina);
                    pln.Real_Feb = ReturnPodaniByKVPGroup(new DateTime(year, 2, 1), new DateTime(year, 2, 28), pln.idKVPSkupina.idKVPSkupina);
                    pln.Real_Mar = ReturnPodaniByKVPGroup(new DateTime(year, 3, 1), new DateTime(year, 3, 31), pln.idKVPSkupina.idKVPSkupina);
                    pln.Real_Apr = ReturnPodaniByKVPGroup(new DateTime(year, 4, 1), new DateTime(year, 4, 30), pln.idKVPSkupina.idKVPSkupina);
                    pln.Real_Maj = ReturnPodaniByKVPGroup(new DateTime(year, 5, 1), new DateTime(year, 5, 31), pln.idKVPSkupina.idKVPSkupina);

                    pln.Real_Jun = ReturnPodaniByKVPGroup(new DateTime(year, 6, 1), new DateTime(year, 6, 30), pln.idKVPSkupina.idKVPSkupina);
                    pln.Real_Jul = ReturnPodaniByKVPGroup(new DateTime(year, 7, 1), new DateTime(year, 7, 31), pln.idKVPSkupina.idKVPSkupina);
                    pln.Real_Avg = ReturnPodaniByKVPGroup(new DateTime(year, 8, 1), new DateTime(year, 8, 31), pln.idKVPSkupina.idKVPSkupina);
                    pln.Real_Sep = ReturnPodaniByKVPGroup(new DateTime(year, 9, 1), new DateTime(year, 9, 30), pln.idKVPSkupina.idKVPSkupina);
                    pln.Real_Okt = ReturnPodaniByKVPGroup(new DateTime(year, 10, 1), new DateTime(year, 10, 31), pln.idKVPSkupina.idKVPSkupina);
                    pln.Real_Nov = ReturnPodaniByKVPGroup(new DateTime(year, 11, 1), new DateTime(year, 11, 30), pln.idKVPSkupina.idKVPSkupina);
                    pln.Real_Dec = ReturnPodaniByKVPGroup(new DateTime(year, 12, 1), new DateTime(year, 12, 31), pln.idKVPSkupina.idKVPSkupina);
                    pln.ts = DateTime.Now.Date;
                    pln.Save();
                }
            }
            catch (Exception ex)
        ...
        public XPCollection<PlanRealizacija> GetPlanRealizationByKVPGroupAndYearWithSumAndYTD(int year, Session currentSession = null)
        {
            try
            {
                UpdateRealKVPCount(year, currentSession);
                ...
                CriteriaOperator filterCriteria = CriteriaOperator.Parse("Leto =" + year + " and [idKVPSkupina.Aktivnost] = 1");

                XPCollection<PlanRealizacija> collection_KVP = new XPCollection<PlanRealizacija>(session, filterCriteria);
                collection_KVP.Sorting.Add(new SortProperty("idKVPSkupina", DevExpress.Xpo.DB.SortingDirection.Ascending));

                if (collection_KVP.Count == 0)
                    return null;
                ...
                plnYtd.idPlanRealizacija = 100;
                KVPSkupina kvpYtdP = new KVPSkupina(session);
                kvpYtdP.idKVPSkupina = 100;
        ...
        public void RefreshKVPRealizationPercentage(Session currentSession = null)
        {
            try
            {
                CriteriaOperator filterCriteria = CriteriaOperator.Parse("Leto =" + DateTime.Now.Year + " and [idKVPSkupina.Aktivnost] = 1");
        ...
        public decimal GetPlanNumberByKVPGroupMonthAndYear(int kvpGroupID, int month, int year, Session currentSession = null)
        {
            try
            {
                XPQuery<PlanRealizacija> planRealization = null;
                ...
                var obj = planRealization.Where(p => p.idKVPSkupina.idKVPSkupina == kvpGroupID && p.Leto == year).FirstOrDefault();

                if (obj != null)
                {
                    switch (month)
                    {
                        case 1: return obj.Plan_Jan;
                        case 2: return obj.Plan_Feb;
                        case 3: return obj.Plan_Mar;
                        case 4: return obj.Plan_Apr;
                        case 5: return obj.Plan_Maj;
                        case 6: return obj.Plan_Jun;
                        case 7: return obj.Plan_Jul;
                        case 8: return obj.Plan_Avg;
                        case 9: return obj.Plan_Sep;
                        case 10: return obj.Plan_Okt;
                        case 11: return obj.Plan_Nov;
                        case 12: return obj.Plan_Dec;
                    }
                }
        ...
        public void OpenNewYear(int year, Session currentSession = null)
        {
            int iLastYear = year-1;

            CriteriaOperator filterCriteria = CriteriaOperator.Parse("Leto =" + iLastYear + " and [idKVPSkupina.Aktivnost] = 1");
            ...
            foreach (PlanRealizacija pln in collection_KVP)
            {
                ...
                PlanRealizacija pr = cloneHelper.Clone<PlanRealizacija>(pln);
                ...
                pr.Plan_Jan = pln.Plan_Dec;
                pr.Plan_Feb = pln.Plan_Dec;
                pr.Plan_Mar = pln.Plan_Dec;
                pr.Plan_Apr = pln.Plan_Dec;
                pr.Plan_Maj = pln.Plan_Dec;
                pr.Plan_Jun = pln.Plan_Dec;
                pr.Plan_Jul = pln.Plan_Dec;
                pr.Plan_Avg = pln.Plan_Dec;
                pr.Plan_Sep = pln.Plan_Dec;
                pr.Plan_Okt = pln.Plan_Dec;
                pr.Plan_Nov = pln.Plan_Dec;
                pr.Plan_Dec = pln.Plan_Dec;


                pr.idVnos = PrincipalHelper.GetUserPrincipal().ID;
                pr.ts = DateTime.Now;
                pr.idKVPSkupina = pln.idKVPSkupina;
